...
public class ShortestPaths<WE extends WeightedEdge> {
	...
	private int chooseVertexForNextShortestPath(boolean[] found) {
		//Find a vertex which has minimum distance among the not-found vertices
	...
	private void debug_showIteration(int iteration, int u) {
		...
		for(int w = 0; w <this.graph().numberOfVertices(); w++) {
			...
			if(this.distance()[w] >= WeightedDirectedAdjacencyListGraph.WEIGHT_INFINITE) {
	...
	public Stack<Integer> pathToDestination(int aDestination){
		Stack<Integer> pathStack = new LinkedStack<Integer> ();
